...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    digit_strings_list = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
    digit_string = ""
    for digit in digit_list:
        digit_string = digit_string + digit_strings_list[int(digit)] + " "
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    camelcase_str = splited_string[0]
    for string in splited_string[1:]:
